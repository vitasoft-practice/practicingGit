...
import { UserDto, UserLoginDto, UserUpdateInfo } from '../controller/user.dto';
...
export class UserService {
  ...
  async updateUserInfo(userId: string, user: UserUpdateInfo) {
    console.log(user);
    const users = await this.userModel.findOneAndUpdate({ userId: userId }, {
      name: user.name,
      age: user.age
    }, {
      new: true
